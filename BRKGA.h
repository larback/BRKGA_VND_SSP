...
extern double tempoBuscas[4]; // VND, SWITCH, US, 2OPT
extern long execucoesBuscas[4];
extern long melhoriasBuscas[4];
extern double mediaMelhorias[4];
using namespace std::chrono;

extern high_resolution_clock::time_point t2_,t1_;
extern duration<double> time_span_;
using namespace std;
...
//   // Elite
  ...
  //  cout << "Aqui \n";
  // melhorou é usado para ver se já foi executado o VND para este cromossomo
    ...
    if (!next.getMelhorou(j)){
      ...
      //  cout << "entrou ";
        ...
        next.setFitness(j, VND(next.population[j]));
		execucoesBuscas[0]++;
		melhoriasBuscas[0]=melhoriasBuscas[0] + (makespan_1 - next.getFitness(j));
		t2_ = high_resolution_clock::now();
		time_span_ = duration_cast<duration<double>>(t2_ - t1_);
		tempoBuscas[0]+=time_span_.count();
